...
const getLinter = (monaco: typeof Monaco, model: IWikitextModel, parserConfig: Config | boolean): void => {
	...
	if ((getObject(storageKey) as string[] | null)?.includes('lint')) {
		...
		(async () => {
			switch (model.getLanguageId()) {
				case 'wikitext': {
					...
					linter.lint = (text): Promise<Monaco.editor.IMarkerData[]> => wikilint.monaco(text);
					break;
				}
				case 'javascript': {
					const esLint: (text: string) => Linter.LintMessage[] = await getJsLinter();
